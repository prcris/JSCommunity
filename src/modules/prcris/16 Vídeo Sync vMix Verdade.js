...
function triggers(module) {
  ...
  arr.push({
    ...
    arr.push({
        id: "video_close_vmix_" + mUID,
        when: "closing",
        item: "any_video",
        action: function(obj) {
            if (gsJump()) {
                h.log(mUID, "======= " + jsc.i18n("Video finished - activating previous scene in vMix") + ": {}", gsJump());
...
function settings() {
    return [
        ...
        {
            id: 'dante_input',
            label: jsc.i18n("Dante audio mixer channel"),
            description: jsc.i18n("Audio mixer channel number to mute while video is on program"),
            type: 'integer',
            min: 1,
            default_value: 20
        },
...
function vmixVideo(module, show, mediaName) { 
    ...
    // 🔹 Verifica se existe versão "_live" do arquivo
    var dotIndex = mediaName.lastIndexOf(".");
    if (dotIndex > 0) {
        var baseName = mediaName.substring(0, dotIndex);
        var extension = mediaName.substring(dotIndex);
        var liveFile = baseName + "_live" + extension;
        if (h.videoExists(liveFile)) {
            h.log(mUID, "======= " + jsc.i18n("Live version found, replacing video with") + ": {}", liveFile);
            mediaName_live = liveFile;
        } else {
            h.log(mUID, "======= " + jsc.i18n("No live version found, keeping original file."));
            mediaName_live = mediaName;
        }
    }
    

    var heigth = getVideoHeigth(mediaName_live);
    var zoom = calculateZoom(s.output_resolution, heigth);
    var url = createURL(pSettings, mediaName_live, s.samePC);
    var previousScene = jsc.vmix.getActiveInputName(p1);
    
    if (previousScene != s.scene_name2 && previousScene != s.scene_name1) { 
    ...
    h.setTimeout(function() {
        ...
        startStopInterval(module, p2, mediaName_live);
    
    h.log(mUID, "======= " + jsc.i18n("Scene activated in vMix") + ", " + jsc.i18n("scene") + ": {}, " + jsc.i18n("file") + ": {}", p2, mediaName_live);

    if (gsJump()) {
        jsc.utils.trigger.addSingleRunVideoOnStop(mediaName_live, function() {
            if (jsc.vmix.getActiveInputName(p1) == p2) { 
                  h.setTimeout(function() { 
                    if (!isPlaying()) {
...
function startStopInterval(module, activeInput, mediaName) {
    var intervalKey = mUID + '_stopInterval';
    var existing = h.getGlobal(intervalKey, null);
    if (existing) {
        h.clearInterval(existing);
        h.setGlobal(intervalKey, null);
    }

    var p1 = module.settings.receiver_id;
    ...
    var id = h.setInterval(function() {
        ...
        if (!lastPlaying) {
            if (jsc.vmix.getActiveInputName(p1) == activeInput) {
                h.log(mUID, "======= " + jsc.i18n("Video finished - activating previous scene in vMix") + ": {}", gsJump());
                restorePreviousScene(module);
                module.updatePanel();
            }
...
function getVideoHeigth(mediaName) {
  ...
  var video = h.hly('GetVideos', {
                   folder : folder,
                   filter : file,
                   include_metadata: true
             })
  h.log(mUID, "folder:{} | file:{}",folder, file);
  h.logp(mUID, "Video info: {}", video.data[0]);
   
  return video.data[0].height;
...
function getPreviewInputName(receiverID) {
    try {
        ...
        var previewIndex = h.tagExtractSingle(status, "<preview>", "</preview>");
        for (var i = 0; i < inputs.length; i++) {
                if (inputs[i].number === previewIndex) {
                        return inputs[i].title;
                }
        }
    } catch (e) {
        h.log(mUID, "Erro getPreviewInputName: {}", [e]);
    }
    return null;
}
...
function restorePreviousScene(module) {
      var s = module.settings;
      var p1 = s.receiver_id
            var previewScene = getPreviewInputName(p1);
            var targetScene = previewScene || gsJump();
            if (!targetScene) {
                    return;
            }
...
function setDanteMute(module, muted) {
    var s = module.settings;
    var p1 = s.receiver_id;
    var input = s.dante_input || '20';
    var fn = muted ? "AudioOff" : "AudioOn";
    try {
        jsc.vmix.request(p1, "Function=" + fn + "&Input=" + encodeURIComponent(input));
        h.log(mUID, "Dante input {} {}", [input, muted ? "muted" : "unmuted"]);
    } catch (e) {
        h.log(mUID, "Erro setDanteMute: {}", [e]);
    }
}

function setSideScreens(module, enabled) {
    var s = module.settings;
    if (!s.side_screens_fullscreen) {
        return;
    }
    var p1 = s.receiver_id;
    var fn = enabled ? "FullscreenOn" : "FullscreenOff";
    try {
        jsc.vmix.request(p1, "Function=" + fn);
        h.log(mUID, "Side screens {}", [enabled ? "on" : "off"]);
    } catch (e) {
        h.log(mUID, "Erro setSideScreens: {}", [e]);
    }
}

/**
 * Envia input para program
 */
function sendToProgram(receiverID, inputName, duration) {
    duration = duration == null ? 500 : duration;
    return jsc.vmix.setInput(receiverID, inputName, "Fade", duration);
}
